...
defmodule ApacheIcebergRESTCatalogAPI.Connection do
  @moduledoc """
  Handle Tesla connections for ApacheIcebergRESTCatalogAPI.

  Additional middleware can be set in the compile-time or runtime configuration:

      config :tesla, ApacheIcebergRESTCatalogAPI.Connection,
        base_url: "https://localhost",
        adapter: Tesla.Adapter.Hackney

  The default base URL can also be set as:

      config :apache_iceberg_rest_catalog_api,
        :base_url, "https://localhost"
  """

  @default_base_url Application.compile_env(
                      :apache_iceberg_rest_catalog_api,
                      :base_url,
                      "https://localhost"
                    )

  @default_scopes [
    # Allows interacting with the Config and Catalog APIs
    "catalog"
  ]
  ...
  def middleware(options \\ []) do
    base_url =
      Keyword.get(
        options,
        :base_url,
        Application.get_env(:apache_iceberg_rest_catalog_api, :base_url, @default_base_url)
      )
    ...
    user_agent =
      Keyword.get(
        ...
        Keyword.get(
          tesla_options,
          :user_agent,
          "openapi-generator - ApacheIcebergRESTCatalogAPI 0.0.1 - elixir"
        )
      )
